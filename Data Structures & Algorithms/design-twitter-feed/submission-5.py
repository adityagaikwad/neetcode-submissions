...
Time:
    getNewsFeed: O(n log n)
        Where n = max followee ids of a user from list of users passed in getNewsFeed
    All others: O(1)
...
Space:
    ...
    n = total number of followees of userId called with getNewsFeed

    N * m for self.tweets
    N * M for self.following
    n for heap in size in getNewsFeed
'''
class Twitter:
...
'''
There is a more optimized solution where we keep max 10 latest tweets per user
and use it to make getNewsFeed O(n)
'''