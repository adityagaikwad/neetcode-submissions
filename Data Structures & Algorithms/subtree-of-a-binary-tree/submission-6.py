...
class Solution:   
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        # An empty tree is always a subtree of anything
        if not subRoot:
            return True
        
        # if subTree exists but not root, then invalid
        if not root:
            return False

        # if base/root is not matching, then check left and right for subtree match
