...
class MedianFinder:
    ...
    Time:
        Overall: O(n * log n)
            n times we call addNum method

        addNum: O(log n) per call
                n = length of the arr/stream so far
                each heappush/heappop is O(log n) on a heap of size n.
                At most 2 push/pop operations happen.
        findMedian: O(1)
    Space:
        O(n) overall for the two heaps
        n = length of the arr/stream so far
    '''
    def __init__(self):
        self.leftMaxHeap = []
