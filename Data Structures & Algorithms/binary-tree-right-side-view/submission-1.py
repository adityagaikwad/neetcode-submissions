...
'''
Use dfs(root, depth) with dfs(root, 0) to start

if len(ans) == depth: the curr node is the first node at
treeDepth = depth + 1. It's like marking visited at depth = depth + 1

then dfs(right, depth + 1) and then dfs(left, depth + 1)
...
#                 node = q.popleft()
#                 if node:
#                     # last assigment to rightSide is rightmost on that level
#                     rightSide = node
